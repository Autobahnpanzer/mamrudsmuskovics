 ...
 * added support for Stamen [Jan Unger]
 *
 * @package		CodeIgniter
...
class Googlemaps {
	...
	var $enableStamenMap			= FALSE;
	var $stamenMapType				= 'toner';
	...
	function create_map()
	{
		...
		'; }
		if ($this->enableStamenMap) { $this->output_js .= '
		...
		if($this->enableStamenMap) {
			$this->output_js_contents .= 'var layer = "' . $this->stamenMapType . '";';
		}
		...
		if (!$this->enableStamenMap && count($this->map_types_available)) {
			foreach ($this->map_types_available as $map_type) { array_push($map_types, 'google.maps.MapTypeId.'.strtoupper($map_type)); }
		}
		if($this->enableStamenMap) {
			$styleOutput .= '
					  '.$this->map_name.'.mapTypes.set(layer, new google.maps.StamenMapType(layer));
				';
		} elseif (count($this->styles) && $this->stylesAsMapTypes) {
			$styles = 0;
			foreach ($this->styles as $style) {
		...
		} elseif (count($mapTypeControlOptions)) {
			$this->output_js_contents .= ',
		...
		if($this->enableStamenMap)		
			$this->output_html .= '<p id="map_credits">Map tiles by <a href="http://stamen.com">Stamen Design</a>, under <a href="http://creativecommons.org/licenses/by/3.0">CC BY 3.0</a>. Data by <a href="http://openstreetmap.org">OpenStreetMap</a>, under <a href="http://creativecommons.org/licenses/by-sa/3.0">CC BY SA</a>.</p>';
