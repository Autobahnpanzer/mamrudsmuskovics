...
class MY_Model extends CI_Model {
    ...
    /**
     * Relations array, this relations will be deleted as well
     * configure / overwrite this array in the model
     * 
     * example for a relations array:
     * (if there is no additional condition set 'additional" to FALSE
     *   // delete relation: resources_objects on resources_objects.object_id = id AND resources_objects.object_type = 'news'
     *   $resources_relation = array(
     *   		'table' 	=> 'resources_objects',
     *   		'fkey' 		=> 'object_id',
     *   		'ref' 		=> 'id',
     *   		'additional' => array('key' => 'object_type' , 'value' => 'news'),
     *   );
     *   
     *   $relations = array($resource_relation);
     * 
     * @var array
     */
    ...
    public function delete($ids){
        ...
        foreach ($ids as $id) {
            ...
            if ($id) {
            	...
            	//delete relations before delete object
                $this->delete_relations($id,$this->relations);
    ...
    public function delete_by($key, $value = FALSE, $orwhere = FALSE){
        ...
        //TODO: get id's for pair of key/value, use $this->delete for removing relations as well 
		// NO! this is a routine for delete relations ;-)
		// seen in coupons, events, news, roles & session admin controller and resources and role_rights model
        ...
        if (! is_array($key)) {
            ...
            $key = array_map('htmlentities', $key); 
            $where_method = $orwhere == TRUE ? 'or_where' : 'where';
            $this->db->$where_method($key)->delete($this->table_name);
        }
    }
